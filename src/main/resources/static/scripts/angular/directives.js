...
angular.module("AAB.directives")
    .service("ItemBloodhound", function () {
        return new Bloodhound({
            datumTokenizer: Bloodhound.tokenizers.whitespace,
            queryTokenizer: Bloodhound.tokenizers.whitespace,

            remote: {
                url: '/api/autocomplete/items/%QUERY.json',
                wildcard: '%QUERY'
            }
        });
    });
...
angular.module("AAB.directives")
    .directive("typeahead", function ($parse) {
        return {
            restrict: "AE",
            controller: function ($scope, $element, $attrs, ItemBloodhound) {
                ...
                $element.typeahead({
                    ...
                    {
                        name: "items",
                        source: ItemBloodhound,
                        display: "displayName"
                    });

                $element.bind("typeahead:select", function (event, suggestion) {
                    suggestion.suggested = true;
                    $parse($attrs.ngSuggestionModel).assign($scope, suggestion);
                    $scope.$apply();
                })
            }
        }
